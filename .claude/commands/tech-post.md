...
- **본문을 풍부하게 구성한다 — 이미지·영상·미리보기를 적극 활용**한다. 글이 텍스트만 길게 이어지지 않도록, 섹션마다 이해를 돕는 시각 자료를 곁들인다.
  ...
  - **유튜브/영상**: 이 블로그 렌더러(marked + DOMPurify)는 **`<iframe>`을 제거**하므로 임베드 플레이어는 렌더되지 않는다. 대신 **썸네일 이미지를 영상 링크로 감싸는** 방식으로 "미리보기+클릭 재생"을 만든다:
    `[![영상 제목](https://img.youtube.com/vi/<VIDEO_ID>/hqdefault.jpg)](https://www.youtube.com/watch?v=<VIDEO_ID>)`
    공식 채널·컨퍼런스 세션·릴리스 소개 등 **신뢰할 수 있는 영상만** 링크한다.
